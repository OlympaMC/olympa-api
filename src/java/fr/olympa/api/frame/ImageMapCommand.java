...
public class ImageMapCommand implements TabExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length >= 2 && args[1].equalsIgnoreCase("reload")) {
			this.plugin.reloadImage(args[0]);
			sender.sendMessage("Image " + args[0] + " reloaded!");
			return true;
		}

		if (args.length >= 2 && args[1].equals("info")) {
			BufferedImage image = this.plugin.loadImage(args[0]);
			if (image == null) {
		...
		if (args.length >= 2 && args[1].equals("download")) {
			if (sender.hasPermission("imagemaps.download")) {
				this.plugin.appendDownloadTask(new ImageDownloadTask(this.plugin.plugin, args[2], args[0], sender));
			} else {
		...
		this.plugin.startPlacing((Player) sender, args[0], fastsend, finalScale);
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
		switch (args.length) {
		case 1:
			return getMatches(args[0], new File(this.plugin.plugin.getDataFolder(), "images").list());
		case 2:
