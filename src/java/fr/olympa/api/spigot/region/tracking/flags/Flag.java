...
public class Flag {
	...
	public ActionResult enters(EntryEvent event) {
		if (greeting != null && event.getReason() != RegionEventReason.JOIN) event.getPlayer().spigot().sendMessage(position, greeting);
		return entry;
	}
	...
	public ActionResult leaves(ExitEvent event) {
		if (farewell != null && event.getReason() != RegionEventReason.JOIN) event.getPlayer().spigot().sendMessage(position, farewell);
		return exit;
	}
