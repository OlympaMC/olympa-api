	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] rawArgs) {
		...
		for (int newArgIndex = 0; newArgIndex < newArgs.length; newArgIndex++) {
			...
			if (potentialParsers.isEmpty())
				...
				List<ArgumentParser<CommandSender>> parsers = potentialParsers.stream().filter(p -> p.hasCache ? p.applyTabWithoutCache(sender, arg).contains(arg) : true).collect(Collectors.toList());
				if (!parsers.isEmpty())
